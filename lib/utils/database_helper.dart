...
class DatabaseHelper {
  ...
  //Elimizde bulunan 'assets/notlar.db' veritabanini kopyalayip telefonun hafizasinda olusturmak icin asagidaki islemler yapildi.
  //Oncelikle telefonun hafizasinda bu kopya veri tabani var mi diye kontrol ettik varsa onu kullaniyor yok ise kopya veritabanini olusturuyor.
...
// open the database
  ...
  Future kategoriListesiniGetir() async {
    var kategorilerMapListesi = await kategorileriGetir();
    var kategoriListesi = [];
    for (Map i in kategorilerMapListesi) {
      kategoriListesi.add(Kategori.fromMap(i as Map<String, dynamic>));
    }
  ...
  Future<List<Map<String, dynamic>>> notlariGetir() async {
    var db = await _getDatabase();
    //Burada not tablosu getirilirken o notun bulundugu kategoriyide getiriyoruz kategori id kullanarak.
    //Ayrica burada klasik sorgu yaptik
    var sonuc = await db.rawQuery(
        "select * from 'not' inner join kategori on kategori.kategoriID = 'not'.kategoriID order by notID Desc;");
    return sonuc;
  }

  Future notListesiniGetir() async {
    var notlarMapListesi = await notlariGetir();
    var notListesi = [];
    for (Map i in notlarMapListesi) {
      notListesi.add(Not.fromMap(i as Map<String, dynamic>));
    }
    return notListesi;
  }
  ...
  Future<int> notEkle(Not not) async {
    var db = await _getDatabase();
    var sonuc =
        await db.insert('not', not.toMap(), nullColumnHack: '${not.notID}');
    return sonuc;
  }
